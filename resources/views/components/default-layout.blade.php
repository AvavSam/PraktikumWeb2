    ...
    <header x-data="{ open: false }"
      ...
      <nav>
        <ul class="hidden sm:flex">
          <li>
            <a href="{{ route('dashboard') }}" class="{{ request()->is('dashboard') ? 'text-black' : 'te+xt-zinc-500' }}
                        block px-2 py-1 rounded font-semibold hover:text-black text-sm">
              Dashboard
            </a>
          </li>
          <li>
            <a href="{{ route('students.index') }}" class="{{ request()->is('students') ? 'text-black' : 'text-zinc-500' }}
                        block px-2 py-1 rounded font-semibold hover:text-black text-sm">
              Students
            </a>
          </li>
          <li>
            <a href="{{ route('majors.index') }}" class="{{ request()->is('majors') ? 'text-black' : 'text-zinc-500' }}
                        block px-2 py-1 rounded font-semibold hover:text-black text-sm">
              Majors
            </a>
          </li>
          <li>
            <a href="{{ route('profile.index') }}" class="{{ request()->is('profile') ? 'text-black' : 'text-zinc-500' }}
                        block px-2 py-1 rounded font-semibold hover:text-black text-sm">
              Profile
            </a>
          </li>
      ...
      <div x-show="open" class="bg-white border border-zinc-300 shadow-lg sm:hidden absolute top-12 right-3">
        <ul class="flex flex-col gap-2 py-2 px-4">
          ...
          <li x-on:click="open = !open">
            <a href="{{ route('students.index') }}" class="block px-4 py-2 text-zinc-600 text-sm hover:bg-gray-100">
              Students
            </a>
          </li>
          <li x-on:click="open = !open">
            <a href="{{ route('majors.index') }}" class="block px-4 py-2 text-zinc-600 text-sm hover:bg-gray-100">
              Majors
            </a>
          </li>
          <li x-on:click="open = !open">
